...
class SoftwareCompilerPipeline:
    ...
    def stage_3_schema_generation(self, system_design: str) -> AppConfiguration:
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.last_metrics["failures"].append(type(e).__name__)
                error_msg = str(e).lower()
                
                # Catch 503s, 429s, and all exhaustion errors
                if any(err in error_msg for err in ["503", "unavailable", "429", "exhausted", "too many"]):
                    self.last_metrics["retries"] += 1
                    wait_time = (attempt + 1) * 10
                    print(f"   [API Rate Limit] Caught {type(e).__name__}. Retrying in {wait_time}s...")
        ...
        for attempt in range(3):
            try:
                ...
                if any(err in error_msg for err in ["503", "unavailable", "429", "exhausted", "too many"]):
                    self.last_metrics["retries"] += 1
                    wait_time = (attempt + 1) * 10
                    print(f"   [Repair API Rate Limit] Retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    print(f"   [FATAL API ERROR]: {e}")
                    raise e
    ...
    def run(self, user_prompt: str):
        self.last_metrics = {"retries": 0, "failures": []}
